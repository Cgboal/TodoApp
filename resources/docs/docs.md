...
The rest of this page is rendered by ClojureScript found in the `src/cljs/todo_app/core.cljs` file.
...
Request middleware functions are located under the `todo-app.middleware` namespace.
...
* Let `mount` know to start the database connection by `require`-ing `todo-app.core` in some other namespace.
